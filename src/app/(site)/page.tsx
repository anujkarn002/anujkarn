...
export default async function Home() {
  ...
  return (
    <main className="relative min-h-screen w-full flex flex-col items-center justify-center font-mono text-white">
      <div className="flex flex-col items-center justify-center w-full px-6 pt-16 pb-12" style={{ minHeight: '70vh' }}>
        <h1 className="text-5xl md:text-7xl font-orbitron font-black text-white mb-2 tracking-tight text-center retro-shadow glow-text-cyan">Anuj Karn</h1>
        ...
        <div className="flex gap-3 items-center justify-center mb-8 mt-2">
          <a href="https://github.com/anujkarn002" target="_blank" rel="noopener noreferrer" className="text-white/70 hover:text-accent transition text-2xl" title="GitHub"><FaGithub /></a>
          <a href="https://linkedin.com/in/anujkarn002" target="_blank" rel="noopener noreferrer" className="text-white/70 hover:text-accent transition text-2xl" title="LinkedIn"><FaLinkedin /></a>
          <a href="/resume/Anuj_Karn_Resume_01_04_2025.pdf" download className="text-white/70 hover:text-accent transition text-2xl" title="Résumé"><FaFileAlt /></a>
        </div>
        <div className="flex flex-col gap-12 w-full max-w-2xl mt-8 mb-6">
          {activities.length > 0 && (
            <section className="hud-panel rounded-lg p-5">
              <h3 className="text-white text-base font-bold retro-section mb-4">Recent activity</h3>
              <ul className="flex flex-col gap-4">
                {activities.slice(0, 4).map((item, i) => (
                  <li key={`${item.type}-${item.id}-${i}`} className="text-base md:text-lg group hover:bg-white/5 transition rounded px-2 -mx-2 py-1 flex flex-col">
              ...
              {experience.map((exp, i) => (
                <li key={i} className="text-base md:text-lg hover:bg-white/5 transition rounded px-2 -mx-2 py-1">
                  <span className="flex items-center gap-2 text-white text-base">
                    {exp.position}
                    {exp.isPresent && (
                      <span className="ml-2 px-2 py-0.5 rounded-full bg-accent/10 text-accent text-xs font-semibold tracking-wide border border-accent/40">Current</span>
                    )}
                  </span>
