...
function fmt(date: string, mode: Mode) {
  const d = new Date(date);
  if (mode === "workshop") return `${String(d.getFullYear()).slice(2)}.${String(d.getMonth() + 1).padStart(2, "0")}`;
  if (mode === "deepfield") return new Intl.DateTimeFormat("en", { month: "short", year: "2-digit" }).format(d);
  return new Intl.DateTimeFormat("en", { month: "short", year: "numeric" }).format(d);
}

export default function Writing({ mode, posts, standalone = false }: { mode: Mode; posts: Post[]; standalone?: boolean }) {
  if (posts.length === 0) return null;
  const heading = NAV_LABELS[mode].writing;
          ...
          <div className="md:col-span-6 flex flex-col gap-6">
            {posts.map((p) => (
              <Link key={p._id} href={`/writing/${p.slug}`} className="group flex flex-col gap-1.5">
                ...
                <div className="label">{fmt(p.publishedAt, mode)}</div>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 py-10 flex flex-col gap-5" style={{ background: "var(--bg)", borderBottom: "1px solid var(--line)" }}>
        ...
        {posts.map((p) => (
          <Link key={p._id} href={`/writing/${p.slug}`} className="flex justify-between gap-6 items-baseline hover:text-accent transition-colors">
            ...
            <span className="label shrink-0">{fmt(p.publishedAt, mode)}</span>
          </Link>
        ))}
      </section>
    );
